...
<body>
    <div class="fam-sel">
        ...
        <?php foreach ($resumen as $h): ?>
            <a class="fam-card" href="../estudiante_ficha.php?id=<?php echo (int) ($h['id'] ?? 0); ?>">
                <strong><?php echo htmlspecialchars(trim(($h['apellido'] ?? '') . ', ' . ($h['nombre'] ?? '')), ENT_QUOTES, 'UTF-8'); ?></strong>
                <span style="font-size:.85rem;color:#64748b;">Ver ficha y boletín</span>
            </a>
        <?php endforeach; ?>
        <div class="fam-out">
            <a href="familia_logout.php"><i class="fas fa-sign-out-alt"></i> Salir</a>
        </div>
    </div>
</body>
</html>
