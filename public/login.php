...
if ($esPost) {
    ...
    if (!$recaptchaConfigured) {
        $error = 'reCAPTCHA no está configurado. Copie config/recaptcha.local.php.example a config/recaptcha.local.php e ingrese las claves de Google reCAPTCHA v2.';
    } else {
        $recaptchaToken = (string) (filter_input(INPUT_POST, 'g-recaptcha-response', FILTER_DEFAULT) ?? '');
        if ($isLocalhost && $recaptchaToken === '') {
            $recaptchaToken = 'bypass_test';
        }
        $remoteIp = isset($_SERVER['REMOTE_ADDR']) ? (string) $_SERVER['REMOTE_ADDR'] : null;
        $recaptchaResult = $recaptchaService->verify($recaptchaToken, $remoteIp);
        if (!$recaptchaResult['success']) {
            $error = 'Verificación reCAPTCHA: ' . $recaptchaResult['error'];
        }
    }
    ...
    if ($error === '') {
        ...
        if ($resultado['success']) {
            if (!empty($_SESSION['must_change_password'])) {
                header('Location: ' . app_base_path('/change_password_obligatorio.php'));
                exit();
            }
...
<body>
    ...
    <div class="lp-side">
        ...
        <!-- Logo sin marco de color — fondo blanco para respetar el logo -->
        <div class="lp-side-logo">
            <img src="<?php echo htmlspecialchars(app_base_path('/img/logo-eest2.png'), ENT_QUOTES, 'UTF-8'); ?>" alt="Logo E.E.S.T. N°2">
        </div>
        <div class="lp-side-title">E.E.S.T. N°2<br>"Educación y Trabajo"</div>
        <div class="lp-side-sub">Sistema Administrativo Educativo</div>
        <div class="lp-side-features">
            <div class="lp-feat">
                ...
                <div class="lp-feat-text">
                    ...
                    <span>Legajos, notas y asistencia</span>
                </div>
            </div>
            <div class="lp-feat">
                ...
                <div class="lp-feat-text">
                    ...
                    <span>Notas, boletines y certificados</span>
                </div>
            </div>
            <div class="lp-feat">
    ...
    <div class="lp-main">
        <div class="lp-card">
            <div class="lp-form-header">
                ...
                <p>
                    <?php if ($googleOauthActivo): ?>
                        <strong>Docentes:</strong> usá «Continuar con Google» (misma cuenta que cargaron en el alta).
                        <strong>Equipo directivo y demás roles:</strong> usuario y contraseña del sistema.
                    <?php else: ?>
                        Ingresá con tu usuario y contraseña. Para docentes con Google, el administrador debe configurar OAuth (archivo de ayuda en <code>config/google_oauth.local.php.example</code>).
                    <?php endif; ?>
                </p>
            </div>
            ...
            <div class="lp-alert lp-alert--warn">
                ...
                <span>Falta configurar reCAPTCHA. Creá <code>config/recaptcha.local.php</code> a partir del archivo de ejemplo.</span>
            </div>
            <?php endif; ?>
            ...
            <form method="POST" class="lp-form" action="">
                ...
                <button type="submit" class="lp-submit"<?php echo $recaptchaConfigured ? '' : ' disabled'; ?>>
            ...
            <div class="lp-google-wrap" style="margin-top:1.35rem;text-align:center;">
                <p style="font-size:.78rem;color:#64748b;margin-bottom:.65rem;">Docentes</p>
            ...
            <div class="lp-footer">
                ...
                <small>E.E.S.T. N°2 · Formando Futuros Profesionales · <?php echo date('Y'); ?></small>
                <a href="../documentacion.php" class="lp-doc-btn">
                    <i class="fas fa-book"></i> Documentación
                </a>
            </div>
            <div class="lp-familia-link">
                ¿Sos padre o familiar? <a href="<?php echo htmlspecialchars(app_base_path('/public/inicio.php'), ENT_QUOTES, 'UTF-8'); ?>#acceso-familias">Portal de familias</a>
            </div>
            <div class="lp-disclaimer" style="margin-top: 1.5rem; font-size: 0.72rem; color: #94a3b8; text-align: center; line-height: 1.45; border-top: 1px dashed rgba(148, 163, 184, 0.22); padding-top: 1rem;">
                <p style="margin-bottom: 0.35rem;"><i class="fas fa-shield-alt" style="color: #3b82f6; margin-right: 0.25rem;"></i> <strong>Aviso de seguridad y privacidad (Ley N° 25.326)</strong></p>
                <p style="margin: 0; padding: 0 0.5rem;">Este sistema contiene datos académicos y personales protegidos por ley. El acceso no autorizado o utilizando credenciales ajenas sin consentimiento está estrictamente prohibido. En caso de discrepancias, prevalecerán siempre los registros físicos y libros de actas de secretaría por sobre la información digitalizada.</p>
            </div>
        </div>
    </div>
