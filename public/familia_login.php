...
if (strtoupper((string) ($_SERVER['REQUEST_METHOD'] ?? '')) !== 'POST') {
    header('Location: inicio.php', true, 302);
    exit;
}

$csrf = (string) (filter_input(INPUT_POST, 'csrf_token', FILTER_DEFAULT) ?? '');
if (!verifyCSRFToken($csrf)) {
    header('Location: inicio.php?familia_error=' . rawurlencode('No se pudo validar la solicitud. Actualizá la página e intentá de nuevo.'), true, 302);
    exit;
}
...
if (strlen($dniNorm) < 7 || strlen($dniNorm) > 8) {
    header('Location: inicio.php?familia_error=' . rawurlencode('Ingresá un DNI válido (7 u 8 dígitos).'), true, 302);
    exit;
}
...
if ($intentos >= 10 && (time() - $ultimo) < 900) {
    header('Location: inicio.php?familia_error=' . rawurlencode('Demasiados intentos. Probá de nuevo en unos minutos.'), true, 302);
    exit;
}
...
if ($matches === []) {
    ...
    header('Location: inicio.php?familia_error=' . rawurlencode('No hay estudiantes asociados a ese DNI. Si recién cargaron tus datos, verificá el número o consultá en secretaría.'), true, 302);
    exit;
}
...
if (count($matches) === 1) {
    header('Location: ../estudiante_ficha.php?id=' . (int) $matches[0]['id'], true, 302);
    exit;
}
