...
$pageTitle = 'Estudiantes - Sistema Administrativo E.E.S.T N°2';
...
<section class="estudiantes-section">
    <div class="section-header">
        ...
        <div class="header-actions" style="display: flex; gap: 0.5rem; align-items: center;">
            ...
            <a href="<?php echo htmlspecialchars(app_base_path('estudiantes.php?action=importar'), ENT_QUOTES, 'UTF-8'); ?>" class="btn btn-secondary" style="background-color: #107c41; color: white; border-color: #107c41; display: inline-flex; align-items: center; gap: 0.35rem;">
                <i class="fas fa-file-excel"></i> <span class="btn-text">Importar Excel</span>
            </a>
            <a href="<?php echo htmlspecialchars(app_base_path('estudiantes.php?action=nuevo'), ENT_QUOTES, 'UTF-8'); ?>" class="btn btn-primary" style="display: inline-flex; align-items: center; gap: 0.35rem;">
                <i class="fas fa-plus"></i> <span class="btn-text">Nuevo Estudiante</span>
            </a>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div class="alert alert-info alert-preceptor">
        <i class="fas fa-info-circle"></i> Vista docente: solo estudiantes de los cursos donde tiene materias asignadas.
    </div>
    ...
    <div class="alert alert-info alert-preceptor">
        <i class="fas fa-info-circle"></i> Vista de preceptor: solo sus cursos asignados
    ...
    <div class="card" style="margin-bottom: 1.5rem; border-left: 4px solid #107c41;">
        <div class="card-header" style="background-color: #f3fcf7;">
            <h3 class="card-title" style="color: #107c41; display: flex; align-items: center; gap: 0.5rem;">
                <i class="fas fa-file-excel"></i> Importar Estudiantes desde Plantilla Excel
            </h3>
        </div>
        <form method="POST" class="form-container" enctype="multipart/form-data">
            ...
            <div class="form-info" style="margin-bottom: 1rem;">
                ...
                <ul style="margin-left: 1.5rem; margin-top: 0.5rem; margin-bottom: 0.5rem; list-style-type: disc;">
                    ...
                    <li>Importará todos los estudiantes listados desde la fila 3 (DNI, Nombre, Apellido y Grupo).</li>
                    <li>Omitirá registros con DNI que ya existan en el sistema (evita duplicados).</li>
                    <li>Establecerá el DNI del estudiante como su DNI de responsable predeterminado para el acceso de familias.</li>
                </ul>
            </div>
            <div class="form-row">
                <div class="form-group" style="flex: 1;">
                    <label for="excel_file">Archivo de Plantilla (.xlsm): <span class="required">*</span></label>
            ...
            <div class="form-actions" style="margin-top: 1rem; display: flex; gap: 0.5rem;">
                ...
                <a href="<?php echo htmlspecialchars(app_base_path('estudiantes.php'), ENT_QUOTES, 'UTF-8'); ?>" class="btn btn-secondary">
        ...
        <form method="POST" class="form-container" enctype="multipart/form-data">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" name="dni" id="dni" required maxlength="20"
                           placeholder="Ej: 12345678" pattern="[0-9]{7,8}"
                           title="Debe tener entre 7 y 8 dígitos"
                           value="<?php echo htmlspecialchars($form_values['dni'], ENT_QUOTES, 'UTF-8'); ?>">
                </div>

                <div class="form-group">
                    <label for="dni_responsable">DNI del responsable (portal familias): <span class="required">*</span></label>
                    <input type="text" name="dni_responsable" id="dni_responsable" required maxlength="20"
                           placeholder="Padre, madre o tutor — mismo DNI para ingresar al portal"
                           pattern="[0-9]{7,8}" title="7 u 8 dígitos"
                           value="<?php echo htmlspecialchars($form_values['dni_responsable'], ENT_QUOTES, 'UTF-8'); ?>">
                    <small class="form-hint">Ese DNI permitirá consultar la ficha y el boletín desde el portal público.</small>
                </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <textarea name="domicilio" id="domicilio" placeholder="Dirección completa"><?php echo htmlspecialchars($form_values['domicilio'], ENT_QUOTES, 'UTF-8'); ?></textarea>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="curso_id" id="curso_id">
                        ...
                        <?php
                            $cursoEspecialidad = $curso['especialidad'] ? $curso['especialidad'] : (((int) ($curso['anio'] ?? 0) < 4) ? 'Ciclo básico' : 'Sin especialidad');
                            $cursoTurno = $curso['turno'] ? $curso['turno'] : 'Sin turno';
                            $cid = (int) $curso['id'];
                        ?>
                        <option value="<?php echo $cid; ?>"
                                <?php echo $form_values['curso_id'] !== '' && $form_values['curso_id'] === (string) $cid ? 'selected' : ''; ?>>
                            <?php echo (int) $curso['anio'] . '° ' . htmlspecialchars((string) ($curso['division'] ?? ''), ENT_QUOTES, 'UTF-8') . ' - ' . htmlspecialchars((string) $cursoEspecialidad, ENT_QUOTES, 'UTF-8') . ' (' . htmlspecialchars((string) $cursoTurno, ENT_QUOTES, 'UTF-8') . ')'; ?>
                        </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="grupo_taller" id="grupo_taller">
                        ...
                        <option value="<?php echo $g; ?>"
                                <?php echo !empty($form_values['grupo_taller']) && $form_values['grupo_taller'] === $g ? 'selected' : ''; ?>>
                            Grupo <?php echo $g; ?>
                        </option>
                        <?php endforeach; ?>
                    </select>
    ...
    <div class="card">
        ...
        <form method="GET" class="form-container">
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <select name="curso" id="curso">
                        ...
                        <option value="<?php echo $cid; ?>"
                            ...
                            <?php echo (int) $curso['anio'] . '° ' . htmlspecialchars((string) ($curso['division'] ?? ''), ENT_QUOTES, 'UTF-8') . ' - ' . htmlspecialchars((string) $cursoEspecialidad, ENT_QUOTES, 'UTF-8'); ?>
                        </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div class="form-group">
                    <label for="grupo_taller_filtro">Grupo de Taller:</label>
                    <select name="grupo_taller" id="grupo_taller_filtro">
                        <option value="">Todos los grupos</option>
                        <option value="sin_grupo" <?php echo $grupoTallerFilter === 'sin_grupo' ? 'selected' : ''; ?>>Sin grupo de taller</option>
                        <?php foreach (['A', 'B', 'C', 'D', 'E'] as $g): ?>
                        <option value="<?php echo $g; ?>" <?php echo $grupoTallerFilter === $g ? 'selected' : ''; ?>>Grupo <?php echo $g; ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Estudiantes Registrados (<?php echo number_format($total_listado_filtrado, 0, ',', '.'); ?>)</h3>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <br><small>
                                    ...
                                    (<?php echo (int) ($estudiante['edad'] ?? 0); ?> años)
                                </small>
                                <?php endif; ?>
                            </td>
                            <td>
                                ...
                                <?php elseif (empty($estudiante['listado_curso_ok'])): ?>
                                    <span class="status status-warning">Curso no encontrado</span>
                                <?php else: ?>
                            ...
                            <td>
                                <?php if (!empty($estudiante['listado_curso_ok'])): ?>
                                    <span class="status status-primary"><?php echo htmlspecialchars((string) $estudiante['listado_curso_turno'], ENT_QUOTES, 'UTF-8'); ?></span>
                                <?php else: ?>
                                    <span class="status status-warning">Sin asignar</span>
                                <?php endif; ?>
                            </td>
                            ...
                            <td>
                                ...
                                <a href="<?php echo htmlspecialchars(app_base_path('estudiante_ficha.php?' . http_build_query(['id' => (int) $estudiante['id']])), ENT_QUOTES, 'UTF-8'); ?>"
                                   class="btn btn-sm btn-primary" title="Ver ficha completa">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <?php if (!$es_profesor_est): ?>
                                <a href="<?php echo htmlspecialchars(app_base_path('llamados.php?' . http_build_query(['estudiante' => (int) $estudiante['id']])), ENT_QUOTES, 'UTF-8'); ?>"
        ...
        <nav class="pagination-nav" aria-label="Paginación de estudiantes" style="display: flex; flex-wrap: wrap; align-items: center; justify-content: center; gap: 0.35rem; padding: 1rem 1rem 1.25rem; border-top: 1px solid var(--medium-gray, #e2e8f0);">
            ...
            $mk = static function (array $base, int $p): string {
                $base['page'] = $p;
                $rel = 'estudiantes.php?' . http_build_query($base);
