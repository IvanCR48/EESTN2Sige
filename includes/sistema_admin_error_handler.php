...
if (!function_exists('sistema_admin_render_friendly_error')) {
    function sistema_admin_render_friendly_error(Throwable $exception, bool $isDev): void
    {
        ...
        echo '<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Error del sistema</title>';
        ...
        echo '<div class="wrap"><div class="card"><h1 class="title">Ups, ocurrio un problema</h1>';
        echo '<p class="msg">No te preocupes, ya registramos el incidente. Contacta a soporte para ayudarte con este caso.</p>';
        echo '<p class="meta">Soporte: <a class="mail" href="mailto:' . htmlspecialchars($supportEmail, ENT_QUOTES, 'UTF-8') . '">' . htmlspecialchars($supportEmail, ENT_QUOTES, 'UTF-8') . '</a><br>ID de error: ' . htmlspecialchars($errorId, ENT_QUOTES, 'UTF-8') . '</p>';
        if ($isDev) {
