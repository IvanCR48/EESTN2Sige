...
/**
 * Consultas y armado del boletín / avances para notas.php (sin SQL en la vista).
 */
class ServicioBoletinNotas extends BaseService
{
    ...
    public function construirBoletin(
        ...
        foreach ($boletinOrganizado as &$datosEstudiante) {
            foreach ($datosEstudiante['notas'] as &$datosMateria) {
                $valores = [];
                $t1 = $datosMateria['cuatrimestres'][1]['nota'] ?? null;
                $t2 = $datosMateria['cuatrimestres'][2]['nota'] ?? null;
                if ($t1 !== null && $t1 !== '') {
                    $valores[] = (float) $t1;
                }
                if ($t2 !== null && $t2 !== '') {
                ...
                if ($valores !== []) {
                    ...
                    $datosMateria['cuatrimestres']['final']['observaciones'] = count($valores) === 2
    ...
    /**
     * Tabla de notas/avances por materia para imprimir_boletin.php (bimestres 1 y 2).
     *
    ...
    public function construirNotasParaImpresionBoletin(int $estudianteId, ?int $cursoId): array
    {
        ...
        $notasRaw = $this->database->fetchAll(
            ...
            WHERE n.estudiante_id = ? AND n.bimestre IN (1, 2)
        ...
        foreach ($notasOrganizadas as &$datos) {
            ...
            if ($notasValidas !== []) {
                ...
                $datos['promedio_completo'] = (count($notasValidas) === 2);
            } else {
                $datos['promedio'] = null;
