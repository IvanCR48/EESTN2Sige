...
class ServicioCursos extends BaseService
{
    ...
    {
        $tid = $curso->getTurnoId();
        if ($tid === null || $tid <= 0) {
            throw new \InvalidArgumentException('Seleccione un turno válido.');
        }
    ...
    public function crearDesdeDatosFormulario(array $post): Curso
    {
        ...
        $turnoId = isset($post['turno_id']) ? (int) $post['turno_id'] : 0;
        $especialidadId = null;
    ...
    /**
     * Listado agregado para cursos.php (mismas columnas que la consulta original).
     *
