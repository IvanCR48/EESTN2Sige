...
class EstudianteController extends BaseService
{
    ...
    private function validarDatosCreacion(array $datos): array
    {
        ...
        } elseif (!preg_match('/^\d{7,8}$/', $datos['dni'])) {
        ...
        } elseif (!preg_match('/^\d{7,8}$/', $dniResp)) {
    ...
    public function actualizarContactoFicha(int $estudianteId, array $datos): array
    {
        try {
            ...
            $dniPortal = null;
            if ($dniPortalRaw !== '') {
                if (!preg_match('/^\d{7,8}$/', $dniPortalNorm)) {
                    return [
                        'success' => false,
    ...
    /**
     * POST de ficha del estudiante con PRG (estudiante_ficha.php).
     *
    ...
    public function procesarPostFichaEstudiante(int $estudianteId, array $post, array $contextoAcceso): array
    {
        ...
        $base = function_exists('app_base_path')
            ? app_base_path('estudiante_ficha.php?id=' . $estudianteId)
            : ('estudiante_ficha.php?id=' . $estudianteId);
    ...
    /**
     * POST alta y baja en estudiantes.php (PRG si tiene éxito).
     *
     * @param array<string, mixed> $post
     * @param list<int> $preceptorCids
     *
    ...
    /**
     * Listado paginado para la vista estudiantes.php (SQL con LIMIT/OFFSET y COUNT paralelo).
     *
     * @param list<int>|null $alcanceCids
     *
     * @return array{
    ...
    private static function etiquetaEspecialidadCursoListado(array $curso): string
    {
        ...
        return ((int) ($curso['anio'] ?? 0) < 4) ? 'Ciclo básico' : 'Sin especialidad';
    }
