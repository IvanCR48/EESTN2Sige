...
class AdminToolsController extends BaseService
{
    ...
    public function obtenerDashboard(): array
    {
        try {
            ...
            return $this->successResponse([
                ...
                'configuracion' => [
                    'sistema' => $this->configService->obtenerPorCategoria('sistema'),
                    'seguridad' => $this->configService->obtenerPorCategoria('seguridad'),
                    'backup' => $this->configService->obtenerPorCategoria('backup'),
                ],
            ]);
    ...
    public function optimizarBaseDatos(): array
    {
        try {
            ...
            $dbName = $_ENV['DB_NAME'] ?? 'sistema_admin_eest2';
    ...
    public function obtenerLogs(string $tipo, int $limite = 100, ?string $filtro = null): array
    {
        try {
            ...
            $filePath = __DIR__ . '/../../logs/' . $tipoMap[$tipo];
