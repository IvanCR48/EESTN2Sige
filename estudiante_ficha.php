...
if (!$usuario && !$familia_portal_lectura) {
    header('Location: ' . app_base_path('public/inicio.php') . '#acceso-familias');
    exit();
}
...
if (empty($fichaVista['success']) || empty($fichaVista['data'])) {
    if ($familia_portal_lectura && !$usuario) {
        header('Location: ' . app_base_path('public/inicio.php'));
    } else {
        header('Location: ' . app_base_path('estudiantes.php'));
    }
    exit();
        ...
        <div class="header-actions">
            <?php if (!empty($familia_portal_lectura) && empty($usuario)): ?>
                ...
                <a href="<?php echo htmlspecialchars(app_base_path('public/familia_seleccion.php'), ENT_QUOTES, 'UTF-8'); ?>" class="btn btn-secondary">
                    <i class="fas fa-users"></i> Elegir otro estudiante
                </a>
                <?php endif; ?>
            ...
            <?php else: ?>
             <?php
             $backUrl = app_base_path('estudiantes.php');
            ...
            <a href="<?php echo htmlspecialchars(app_base_path('llamados.php?' . http_build_query(['action' => 'nuevo', 'estudiante' => $estudiante['id']])), ENT_QUOTES, 'UTF-8'); ?>" class="btn btn-danger">
                <i class="fas fa-exclamation-triangle"></i> Nuevo Llamado
            </a>
            <?php endif; ?>
            <?php if ($es_profesor_ficha || hasRole('admin') || hasRole('directivo')): ?>
            <a href="<?php echo htmlspecialchars(app_base_path('notas.php?' . http_build_query(['estudiante' => (int) $estudiante['id']])), ENT_QUOTES, 'UTF-8'); ?>" class="btn btn-primary">
            ...
            <div class="student-profile">
                ...
                <div class="profile-info">
                    ...
                    <div class="info-grid">
                        ...
                        <div class="info-item">
                            <strong>Grupo Taller:</strong>
                            <span>
                                <?php if (!empty($estudiante['grupo_taller'])): ?>
                                    <span class="status status-primary">Grupo <?php echo htmlspecialchars((string) $estudiante['grupo_taller']); ?></span>
                                <?php else: ?>
                                    <span class="status status-warning">Sin grupo</span>
                                <?php endif; ?>
                            </span>
                        </div>
                        <div class="info-item">
                        ...
                        <div class="info-item">
                            ...
                            <span><?php echo $obraSocial !== '' ? htmlspecialchars((string)$obraSocial) : 'No registrada'; ?></span>
                        </div>
                        <div class="info-item">
            ...
            <div class="contact-info">
                ...
                <div class="info-grid">
                    <div class="info-item">
                        ...
                        <span><?php echo $domicilio !== '' ? htmlspecialchars((string)$domicilio) : 'No registrado'; ?></span>
                    </div>
                    <div class="info-item">
                        <strong>Teléfono Fijo:</strong>
                        <?php $telefonoFijo = $estudiante['telefono'] ?? ''; ?>
                        <span><?php echo $telefonoFijo !== '' ? htmlspecialchars((string)$telefonoFijo) : 'No registrado'; ?></span>
                    </div>
                    <div class="info-item">
                        <strong>Teléfono Celular:</strong>
                        <?php $telefonoCel = $estudiante['telefono_celular'] ?? ($estudiante['telefono'] ?? ''); ?>
                        <span><?php echo $telefonoCel !== '' ? htmlspecialchars((string)$telefonoCel) : 'No registrado'; ?></span>
                    </div>
                    <div class="info-item">
                        <strong>Email:</strong>
                        <?php $emailVal = $estudiante['email'] ?? ''; ?>
                        <span><?php echo $emailVal !== '' ? htmlspecialchars((string)$emailVal) : 'No registrado'; ?></span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card">
            ...
            <div class="stat-content">
                ...
                <small class="ficha-muted"><?php echo htmlspecialchars((string) $porcentaje_aprobadas); ?>% del curso</small>
            </div>
        </div>
        <?php endif; ?>
        ...
        <div class="stat-card">
            ...
            <div class="stat-content">
                ...
                <p>Asistencia anual</p>
                <small class="ficha-muted"><?php echo (int)($asistenciaFicha['resumen']['total'] ?? 0); ?> registros · <?php echo (int)($asistenciaFicha['resumen']['presente'] ?? 0) + (int)($asistenciaFicha['resumen']['tardanza'] ?? 0); ?> presencias</small>
            </div>
        </div>
        <?php endif; ?>
    </div>
    ...
    <div class="ficha-grid ficha-grid--1">
        ...
        <div class="card">
            <div class="card-header ficha-card-header-row">
                ...
                <a href="<?php echo htmlspecialchars(app_base_path('llamados.php?' . http_build_query(['estudiante' => (int) $estudiante['id']])), ENT_QUOTES, 'UTF-8'); ?>" class="btn btn-sm btn-secondary">Ver todos</a>
                <?php endif; ?>
            </div>
            <div class="card-body">
                ...
                <?php else: ?>
                    <p class="text-center-muted">No hay llamados registrados</p>
                <?php endif; ?>
            </div>
        </div>
    </div>
                ...
                <div class="ficha-persona-card responsable-item">
                    ...
                    <h4 class="ficha-persona-card__title">
                        ...
                        <?php if ($responsable['es_contacto_emergencia']): ?>
                            <span class="status status-success ficha-badge-emergencia">Contacto de Emergencia</span>
                        <?php endif; ?>
                    </h4>
                    <div class="info-grid ficha-persona-card__grid">
                        ...
                        <div><strong>Teléfono:</strong> <?php echo htmlspecialchars($responsable['telefono_celular'] ?? ''); ?></div>
...
<div class="card ficha-asistencia-card" style="margin-top:1.25rem">
    <div class="card-header" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:.75rem">
        <h3 class="card-title"><i class="fas fa-calendar-check"></i> Reporte acumulado — año <?php echo date('Y'); ?></h3>
        <form method="get" class="ficha-asistencia-filtros">
            <input type="hidden" name="id" value="<?php echo $estudiante_id_int; ?>">
            <label for="asistencia_materia_id" class="ficha-asistencia-filtros__label">Materia</label>
            ...
            <label for="asistencia_trimestre" class="ficha-asistencia-filtros__label">Cuatrimestre</label>
            <select id="asistencia_trimestre" name="asistencia_trimestre" class="ficha-asistencia-filtros__select">
                <option value="" <?php echo $asistenciaCuatrimestre === '' ? 'selected' : ''; ?>>Ciclo lectivo</option>
                <option value="1" <?php echo $asistenciaCuatrimestre === '1' ? 'selected' : ''; ?>>1er cuatrimestre</option>
                <option value="2" <?php echo $asistenciaCuatrimestre === '2' ? 'selected' : ''; ?>>2do cuatrimestre</option>
            </select>
    ...
    <div class="card-body">
        ...
        $pctLbl = $pctA < 0 ? 'Sin datos' : number_format($pctA, 1) . '%';
        ?>
        <div class="ficha-asistencia-resumen">
            <div class="far-item"><span class="far-value"><?php echo (int)$resA['presente']; ?></span><small class="far-label">Presentes</small></div>
            <div class="far-item"><span class="far-value"><?php echo (int)$resA['tardanza']; ?></span><small class="far-label">Tardanzas</small></div>
            <div class="far-item"><span class="far-value"><?php echo (int)$resA['media_falta']; ?></span><small class="far-label">Media falta</small></div>
            <div class="far-item"><span class="far-value"><?php echo (int)$resA['ausente_justificado']; ?></span><small class="far-label">Justificados</small></div>
            <div class="far-item"><span class="far-value"><?php echo (int)$resA['ausente']; ?></span><small class="far-label">Ausentes</small></div>
            <div class="far-item far-item--total">
                <span class="far-rate"><?php echo htmlspecialchars($pctLbl, ENT_QUOTES, 'UTF-8'); ?></span>
                <small class="far-label">de <?php echo $totalA; ?> clases</small>
            </div>
        </div>

        <!-- Historial reciente -->
        <?php if (!empty($asistenciaFicha['historial'])): ?>
        <h4 class="ficha-asistencia-subtitle"><i class="fas fa-history"></i> Últimos registros</h4>
        <div class="table-container">
            <table class="table ficha-asistencia-table" style="font-size:.82rem">
                <thead><tr><th>Fecha</th><th>Materia</th><th>Estado</th><th>Observación</th><th>Justificativo</th></tr></thead>
                <tbody>
                <?php
                $estadosBadge = [
                    ...
                    <td style="text-align: center;">
                        <?php if ($adj !== null): ?>
                            <a href="<?php echo htmlspecialchars(app_base_path('uploads/' . ltrim((string) $adj, '/')), ENT_QUOTES, 'UTF-8'); ?>" target="_blank" title="Ver justificativo" style="color:var(--primary-color);">
                                <i class="fas fa-file-medical-alt"></i> Ver
                            </a>
                        <?php else: ?>
        ...
        <?php if ($pctA >= 0 && $pctA < 75): ?>
        <div class="alert alert-error" style="margin-top:1rem">
            <i class="fas fa-exclamation-triangle"></i>
            <strong>Riesgo de repitencia por inasistencias.</strong> Porcentaje actual: <?php echo number_format($pctA, 1); ?>% (mínimo requerido: 75%)
        </div>
        <?php endif; ?>
    </div>
</div>
