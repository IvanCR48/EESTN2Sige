...
if ($curso_info_boletin !== null) {
    $espNombreBoletin = !empty($curso_info_boletin['especialidad'])
        ...
        : 'Sin especialidad';
    $titulo_boletin_linea = (string) $curso_info_boletin['anio'] . '° ' . (string) $curso_info_boletin['division'] . ' - ' . $espNombreBoletin;
} else {
...
<section class="notas-section">
    <div class="section-header">
        ...
        <?php endif; ?>
        <?php
        $mostrar_modal_recup = $puede_gestionar_notas && $curso_filter !== '' && $total_estudiantes > 0 && $materias !== [];
        ?>
        <?php if ($mostrar_modal_recup): ?>
        <button type="button" class="btn btn-secondary" id="btn-abrir-modal-recuperatorio">
            <i class="fas fa-book-medical"></i> Cargar recuperatorio
        </button>
        <?php endif; ?>
        <?php if ($estudiante_filter !== '' && $estudiante_info !== null): ?>
        <a href="imprimir_boletin.php?id=<?php echo (int) $estudiante_info['id']; ?>&csrf_token=<?php echo htmlspecialchars($csrfToken, ENT_QUOTES, 'UTF-8'); ?>"
           target="_blank"
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card">
            <div class="stat-icon warning">
                <a href="materias_previas.php" class="notas-stat-link">
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">
                ...
                <?php if ($estudiante_filter && isset($estudiante_info)): ?>
                    <span class="badge badge-info notas-filtro-badge">
                        Estudiante: <?php echo htmlspecialchars($estudiante_info['apellido'] . ', ' . $estudiante_info['nombre'], ENT_QUOTES, 'UTF-8'); ?>
                    </span>
                <?php endif; ?>
            </h3>
        </div>
        <form method="GET" class="form-container">
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    <label for="boletin_cuatrimestre_filtro">Período</label>
                    <select name="trimestre" id="boletin_cuatrimestre_filtro">
                        <option value="">Todos los cuatrimestres</option>
                        <option value="avance1" <?php echo $cuatrimestre_filter === 'avance1' ? 'selected' : ''; ?>>1° Informe de Avance</option>
                        <option value="1" <?php echo $cuatrimestre_filter == '1' ? 'selected' : ''; ?>>1° Cuatrimestre</option>
                        <option value="avance2" <?php echo $cuatrimestre_filter === 'avance2' ? 'selected' : ''; ?>>2° Informe de Avance</option>
                        <option value="2" <?php echo $cuatrimestre_filter == '2' ? 'selected' : ''; ?>>2° Cuatrimestre</option>
                        <option value="final" <?php echo $cuatrimestre_filter == 'final' ? 'selected' : ''; ?>>Nota Final (Promedio)</option>
                    </select>
                </div>
            </div>
            <div class="form-row">
                <div class="form-group">
                    <p class="form-label-like form-label-heading-notas" id="info_cuatrimestre_heading">Información</p>
                    <div id="info_cuatrimestre" class="form-info-box form-info-box-notas" role="note" aria-labelledby="info_cuatrimestre_heading">
                        ...
                        Se mostrarán automáticamente los 2 cuatrimestres (1° y 2°) para poder cargar y editar todas las notas.
                    </div>
                </div>
        </div>
    ...
    <div class="card card--boletin">
        <div class="card-header">
                         <h3 class="card-title">
                     📋 Boletín Completo - <?php echo htmlspecialchars($titulo_boletin_linea, ENT_QUOTES, 'UTF-8'); ?> - Todos los Cuatrimestres
             </h3>
        </div>
        ...
        <div class="boletin-container" id="boletin-scroll-area">
            <table class="boletin-table boletin-table--vertical">
                <thead>
                    <tr>
                        <th class="estudiante-col">Estudiante</th>
                        <th class="materia-name-col">Materia</th>
                ...
                <tbody>
                    ...
                    $backToUrl = $backQuery !== [] ? 'notas.php?' . http_build_query($backQuery) : 'notas.php';
                    ?>
                        ...
                        <td class="estudiante-cell" rowspan="<?php echo $totalMaterias; ?>">
                            <div class="estudiante-cell__head">
                                <strong class="estudiante-cell__nombre">
                                    <a href="estudiante_ficha.php?id=<?php echo (int) $fila['estudiante_id']; ?>&back_to=<?php echo urlencode($backToUrl); ?>" class="estudiante-link" title="Ver ficha del estudiante">
                                        <?php echo $estudianteNombre; ?>
                                    </a>
                                </strong>
                                <a href="imprimir_boletin.php?id=<?php echo (int) $fila['estudiante_id']; ?>&csrf_token=<?php echo htmlspecialchars($csrfToken, ENT_QUOTES, 'UTF-8'); ?>"
                                   target="_blank"
                                ...
                                <?php if (!empty($mostrar_modal_recup)): ?>
                                <button type="button"
                                        class="estudiante-cell__recup-btn js-open-recup-modal"
                                        data-estudiante-id="<?php echo (int) $fila['estudiante_id']; ?>"
                                        title="Cargar recuperatorio"
                                        aria-label="Cargar recuperatorio">
                                    <i class="fas fa-book-medical" aria-hidden="true"></i>
                                </button>
                                <?php endif; ?>
                            </div>
                            <small class="estudiante-cell__dni">DNI: <?php echo $estudianteDni; ?></small>
                        </td>
                        <?php endif; ?>
                                    ...
                                    <form method="POST" class="avance-form">
                                        ...
                                        <div class="avance-input-group">
                                            <select name="valor" class="avance-select">
                                                ...
                                                <option value="TEA" <?php echo $celda['valor_actual'] === 'TEA' ? 'selected' : ''; ?>>TEA</option>
                                                <option value="TEP" <?php echo $celda['valor_actual'] === 'TEP' ? 'selected' : ''; ?>>TEP</option>
                                                <option value="TED" <?php echo $celda['valor_actual'] === 'TED' ? 'selected' : ''; ?>>TED</option>
                                            </select>
                                        </div>
    ...
    <div class="card">
        <div class="card-body">
            <p class="text-center">Aún no hay estudiantes en este curso.</p>
        </div>
    </div>
    <?php endif; ?>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <select name="tipo_registro" id="tipo_registro" required>
                        <option value="numerica" <?php echo $form_values['tipo_registro'] === 'numerica' ? 'selected' : ''; ?>>Nota numérica</option>
                        <option value="avance" <?php echo $form_values['tipo_registro'] === 'avance' ? 'selected' : ''; ?>>Avance (TEA / TEP / TED)</option>
                    </select>
                </div>
    ...
    <?php if (!empty($mostrar_modal_recup)): ?>
    <div id="modal-recuperatorio-notas" class="notas-modal-overlay" role="dialog" aria-modal="true" aria-labelledby="modal-recup-title">
        <div class="notas-modal-box">
            <div class="notas-modal-box__head">
                <h3 id="modal-recup-title" class="notas-modal-box__title">Cargar recuperatorio / intensificación</h3>
                <button type="button" class="notas-modal-close" data-close-recup-modal aria-label="Cerrar">&times;</button>
            </div>
            <form method="post" class="notas-modal-form">
                <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($csrfToken, ENT_QUOTES, 'UTF-8'); ?>">
                <input type="hidden" name="curso_id" value="<?php echo (int) $curso_filter; ?>">
                <div class="form-group">
                    <label for="recup_estudiante_id">Estudiante</label>
                    <select name="estudiante_id" id="recup_estudiante_id" required>
                        <option value="">Seleccionar</option>
                        <?php foreach ($estudiantes_modal_recup as $estModal): ?>
                        <option value="<?php echo (int) $estModal['id']; ?>">
                            <?php echo htmlspecialchars($estModal['apellido'] . ', ' . $estModal['nombre'], ENT_QUOTES, 'UTF-8'); ?>
                        </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group">
                    <label for="recup_materia_id">Materia</label>
                    <select name="materia_id" id="recup_materia_id" required>
                        <option value="">Seleccionar</option>
                        <?php foreach ($materias as $matModal): ?>
                        <option value="<?php echo (int) $matModal['id']; ?>">
                            <?php echo htmlspecialchars((string) $matModal['nombre'], ENT_QUOTES, 'UTF-8'); ?>
                        </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group">
                    <label for="recuperacion_contexto">Contexto de evaluación</label>
                    <select name="recuperacion_contexto" id="recuperacion_contexto" required>
                        <option value="intensification_first_semester">Intensif. 1° cuatrimestre</option>
                        <option value="intensification_december">Diciembre</option>
                        <option value="intensification_february_march" selected>Febrero / Marzo</option>
                    </select>
                </div>
                <div class="form-group">
                    <label for="recuperacion_alcance">Alcance</label>
                    <select name="recuperacion_alcance" id="recuperacion_alcance" required>
                        <option value="first_semester">1° Cuatrimestre</option>
                        <option value="second_semester">2° Cuatrimestre</option>
                        <option value="both" selected>Ambos</option>
                    </select>
                </div>
                <div class="form-group">
                    <label for="calificacion_recup">Calificación (1 a 10)</label>
                    <input type="number" name="calificacion_recup" id="calificacion_recup" min="1" max="10" step="0.01" required>
                </div>
                <div class="form-group">
                    <label for="observaciones_recup">Observaciones (opcional)</label>
                    <input type="text" name="observaciones_recup" id="observaciones_recup" maxlength="500">
                </div>
                <div class="notas-modal-form__actions">
                    <button type="submit" name="guardar_recuperatorio" value="1" class="btn btn-primary">
                        <i class="fas fa-save"></i> Guardar
                    </button>
                    <button type="button" class="btn btn-secondary" data-close-recup-modal>Cancelar</button>
                </div>
            </form>
        </div>
    </div>
...
$notasRecupVersion = @filemtime(__DIR__ . '/js/notas_recuperatorio_modal.js') ?: time();
?>
...
<?php if (!empty($mostrar_modal_recup)): ?>
<script src="js/notas_recuperatorio_modal.js?v=<?php echo rawurlencode((string) $notasRecupVersion); ?>" defer nonce="<?php echo $notasNonce; ?>"></script>
<?php endif; ?>
