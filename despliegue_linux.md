# 🚀 Guía de Despliegue — Sistema Admin EEST2
...
```
```sql
CREATE DATABASE IF NOT EXISTS sistema_admin_eest2 CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
CREATE USER 'tu_usuario_mysql'@'localhost' IDENTIFIED BY 'tu_contraseña';
GRANT ALL PRIVILEGES ON sistema_admin_eest2.* TO 'tu_usuario_mysql'@'localhost';
FLUSH PRIVILEGES;
EXIT;
```
...
```bash
mysql -u tu_usuario_mysql -p sistema_admin_eest2 < database/sistema_admin_eest2.sql
```

Si el archivo es grande y tarda:
```bash
mysql -u tu_usuario_mysql -p --max_allowed_packet=256M sistema_admin_eest2 < database/sistema_admin_eest2.sql
```
...
php -r "new PDO('mysql:host=localhost;dbname=sistema_admin_eest2', 'usuario', 'contraseña'); echo 'DB OK';"
...
- [ ] Base de datos importada (`sistema_admin_eest2.sql`)
