#!/bin/bash
# Script de restauración para Sistema Admin EEST2
...
case $BACKUP_TYPE in
    "db")
        ...
        sleep 10
        docker-compose exec -T database mysql -u root -p${DB_ROOT_PASSWORD:-SecureRootPassword123!} ${DB_NAME:-sistema_admin_eest2} < $BACKUP_FILE
