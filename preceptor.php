...
$preceptorPageUrl = function_exists('app_base_path') ? app_base_path('preceptor.php') : 'preceptor.php';
$preceptorNuevoUrl = function_exists('app_base_path') ? app_base_path('preceptor.php?action=nuevo') : 'preceptor.php?action=nuevo';
...
<section class="preceptores-section">
    <div class="section-header">
        <h2>Gestión de Preceptores</h2>
        <div class="header-actions">
            <a href="<?php echo htmlspecialchars($preceptorNuevoUrl, ENT_QUOTES, 'UTF-8'); ?>" class="btn btn-primary">
                <i class="fas fa-user-plus"></i> <span class="btn-text">Nuevo preceptor</span>
            </a>
            <a href="<?php echo htmlspecialchars(app_base_path('equipo.php'), ENT_QUOTES, 'UTF-8'); ?>" class="btn btn-secondary">
                <i class="fas fa-user-tie"></i> <span class="btn-text">Equipo directivo</span>
            </a>
        </div>
    </div>
    ...
    <?php if (!$tabla_preceptor_curso_ok): ?>
        <div class="alert alert-warning preceptor-alert--spaced">
            ...
            <strong>Configuración pendiente.</strong> Falta crear en la base de datos la tabla que vincula preceptores con cursos.
            Quien administre el servidor debe importar <strong>una sola vez</strong> el archivo de migración del proyecto (carpeta <code>database/migrations</code>, nombre similar a <code>20250325_preceptor_curso</code>) desde phpMyAdmin u otra herramienta, sobre la base de datos del colegio.
        </div>
    <?php endif; ?>
    ...
    <div class="card preceptor-alta-card">
        <div class="card-header">
            <h3 class="card-title">Registrar nuevo preceptor</h3>
        </div>
        <div class="card-body">
            <div class="alert alert-info preceptor-alert--spaced">
                <i class="fas fa-info-circle"></i>
                Se creará un usuario con contraseña temporal aleatoria (se mostrará una sola vez al finalizar el alta).
                El login será <strong>preceptor</strong> seguido de un número (preceptor1, preceptor2, …). El curso elegido queda como primer curso de trabajo.
            </div>
            <form method="post" action="<?php echo htmlspecialchars($preceptorPageUrl, ENT_QUOTES, 'UTF-8'); ?>" class="form-container">
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label for="preceptor_apellido">Apellido: *</label>
                    ...
                    <div class="form-group">
                        <label for="preceptor_nombre">Nombre: *</label>
                    ...
                    <div class="form-group">
                        <label for="preceptor_curso_id">Curso de trabajo: *</label>
                        <select name="curso_id" id="preceptor_curso_id" required>
                            ...
                            <option value="<?php echo (int) $c['id']; ?>" <?php echo isset($_POST['curso_id']) && (string) $_POST['curso_id'] === (string) $c['id'] ? 'selected' : ''; ?>>
                                <?php echo htmlspecialchars(
                                    (string) $c['anio'] . '° ' . (string) ($c['division'] ?? '') . (!empty($c['especialidad']) ? ' — ' . (string) $c['especialidad'] : ''),
                                    ENT_QUOTES,
                                    'UTF-8'
                                ); ?>
                            </option>
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label for="preceptor_telefono">Teléfono</label>
                    ...
                    <div class="form-group">
                        <label for="preceptor_email">Email</label>
                    ...
                    <div class="form-group">
                        <label for="preceptor_foto">URL foto</label>
                ...
                <div class="form-actions">
                    <button type="submit" name="guardar_preceptor" value="1" class="btn btn-primary">
                        <i class="fas fa-save"></i> Guardar preceptor
                    </button>
                    <a href="<?php echo htmlspecialchars($preceptorPageUrl, ENT_QUOTES, 'UTF-8'); ?>" class="btn btn-secondary">Cancelar</a>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card">
            ...
            <div class="stat-content">
                ...
                <p>Cursos activos en el sistema</p>
            </div>
        </div>
    </div>
    ...
    <?php if (empty($preceptores) && !$mostrarFormAltaPreceptor): ?>
        <div class="card">
            <div class="card-body preceptor-empty-card-body">
                ...
                <h3 class="preceptor-empty-title">No hay preceptores activos</h3>
                <p class="text-muted preceptor-empty-lead">
                    Registrá un preceptor con su <strong>curso de trabajo</strong>; podés asignar más cursos después desde esta misma pantalla.
                </p>
                <a href="<?php echo htmlspecialchars($preceptorNuevoUrl, ENT_QUOTES, 'UTF-8'); ?>" class="btn btn-primary">
                    <i class="fas fa-plus"></i> Nuevo preceptor
                </a>
            </div>
        </div>
    <?php else: ?>
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">Preceptores y cursos de trabajo (<?php echo number_format($total_preceptores, 0, ',', '.'); ?>)</h3>
            </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <?php if (empty($p['telefono']) && empty($p['email'])): ?>
                                            <span class="status status-warning" style="font-size: 0.75rem;">Sin datos</span>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <?php if ($cursosQuitar === []): ?>
                                            <span class="status status-warning" style="font-size: 0.75rem;">Sin cursos asignados</span>
                                        <?php else: ?>
                                        ...
                                        <?php if (hasRole('admin')): ?>
                                            <?php if (!empty($p['usuario_login'])): ?>
                                                <button type="button" class="btn btn-secondary btn-sm preceptor-creds-btn" data-csp-toggle-credenciales="<?php echo (int) $eid; ?>">
                                                    <i class="fas fa-eye"></i> Ver credenciales
                                                </button>
                                                <div id="credenciales-<?php echo (int) $eid; ?>" class="preceptor-creds-panel credenciales-info">
                                                    <small class="preceptor-creds-line"><strong>Usuario:</strong> <?php echo htmlspecialchars((string) $p['usuario_login'], ENT_QUOTES, 'UTF-8'); ?></small>
                                                    <small class="preceptor-creds-hint">La contraseña temporal solo se muestra al momento del alta.</small>
                                                </div>
                                            <?php else: ?>
                                                <small class="preceptor-creds-sin">Sin usuario de acceso registrado.</small>
                                            <?php endif; ?>
                                        <?php endif; ?>
                                        <details class="preceptor-details">
                                            <summary class="btn btn-primary preceptor-details-summary preceptor-manage-summary">
                                                <i class="fas fa-layer-group"></i> Gestionar
                                            </summary>
                                            <div class="preceptor-details-panel">
                                                <div class="preceptor-panel-block">
                                                    <h4 class="preceptor-panel-title"><i class="fas fa-minus-circle"></i> Quitar curso</h4>
                                                    <?php if ($cursosQuitar === []): ?>
                                                        <p class="preceptor-panel-hint">No hay cursos asignados para quitar.</p>
                                                    <?php else: ?>
                                                ...
                                                <div class="preceptor-panel-block preceptor-panel-block--add">
                                                    <h4 class="preceptor-panel-title"><i class="fas fa-plus-circle"></i> Agregar curso</h4>
                                                    <?php if ($cursosParaAgregar === []): ?>
                                                        <p class="preceptor-panel-hint">Ya tiene todos los cursos activos del sistema.</p>
                                                    <?php else: ?>
                                                        <form method="post" action="<?php echo htmlspecialchars($preceptorPageUrl, ENT_QUOTES, 'UTF-8'); ?>" class="form-container preceptor-add-form">
                                                            ...
                                                            <div class="form-row preceptor-add-row">
                                                                <div class="form-group preceptor-form-group-curso">
                                                                    <label class="preceptor-sr-only" for="curso_add_<?php echo (int) $eid; ?>">Curso</label>
